...
try:
    ...
    """).fetchall()
    if not rows:
        print(f'{C}[INFO]{NC} ღია DCA პოზიცია არ არის')
    else:
        total_inv = sum(float(r[3]) for r in rows)
        print(f'{C}[INFO]{NC} სულ ღია: {len(rows)} | ინვესტირებული: {total_inv:.2f} USDT')
        for r in rows:
...
# ─────────────────────────────────────────
section "13. SHORT პოზიციები (Futures)"
# ─────────────────────────────────────────
python3 - "$DB_PATH" << 'PYEOF'
import sqlite3, sys
db = sys.argv[1]
...
try:
    ...
    if not closed_s:
        ...
        print(f'{C}[INFO]{NC} ბოლო SHORT-ები:')
        for r in closed_s:
...
# ─────────────────────────────────────────
section "14. futures_engine import + FUTURES ENV"
# ─────────────────────────────────────────
...
for k, exp in [('FUTURES_ENABLED','true'),('FUTURES_MODE','DEMO'),('FUTURES_QUOTE',None),('FUTURES_TP_PCT',None),('FUTURES_SL_PCT',None)]:
    ...
    if k == 'FUTURES_ENABLED':
        col = G if v.lower() == 'true' else Y
        print(f'{col}[{"OK" if v.lower()=="true" else "WARN"}]{NC}   {k}={v}')
    else:
